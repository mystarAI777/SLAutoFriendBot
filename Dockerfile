...
# 起動スクリプトを作成（公式READMEの手順に従う）
RUN echo '#!/bin/bash\n\
set -e\n\
\n\
echo "=== Starting VOICEVOX Engine (Official Docker Image) ==="\n\
\n\
# VOICEVOXエンジンをバックグラウンドで起動（公式と同じパターン）\n\
cd /opt/voicevox_engine\n\
echo "🚀 Starting VOICEVOX engine on 0.0.0.0:50021..."\n\
python run.py --host 0.0.0.0 --port 50021 &\n\
VOICEVOX_PID=$!\n\
\n\
# 起動を待つ（README通りのヘルスチェック）\n\
