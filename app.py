...
def repair_missing_id_sequences():
    ...
    try:
        with engine.connect() as conn:
            targets = conn.execute(text(
                ...
                "  AND pg_get_serial_sequence(c.table_name, 'id') IS NULL "
    ...
    for tbl, dtype in targets:
        try:
            with engine.connect() as conn:
                with conn.begin():
                    # 1. 整数型でなければ bigint へ (hololive_news の double precision 対策)
                    ...
                    # 2. id が NULL の行に連番を振る (ctid で行を一意特定)
                    ...
                    # 4. IDENTITY 付与 (自動採番の復活)
                    conn.execute(text(
                        f"ALTER TABLE {tbl} ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY"
                    ))

                    # 5. 次の採番値を MAX(id)+1 に合わせる
                    nextid = conn.execute(text(
                        f"SELECT COALESCE(MAX(id), 0) + 1 FROM {tbl}"
                    )).scalar()
                    conn.execute(text(
                        f"ALTER TABLE {tbl} ALTER COLUMN id RESTART WITH {int(nextid)}"
                    ))

            ok += 1
            logger.info(f"  ✅ {tbl}: id列の自動採番を復旧 (次のid={int(nextid)})")
        except Exception as e:
            ng += 1
            logger.error(f"  ❌ {tbl}: 修復失敗のためスキップ: {str(e)[:150]}")
