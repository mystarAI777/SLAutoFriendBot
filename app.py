            ...
            for kw in rule['keywords']:
                if kw.lower() in msg_lower or kw in message:
                    ...
                    if entry:
                        ...
                        # ★ v33.21: ORMのsession.add()を使うと、id=NULLの壊れた
                        #   オブジェクトがセッションに滞留し、以降の全クエリが
                        #   autoflushで "NULL identity key" を投げ続ける
                        #   (→ chat_lslがフォールバック応答になり音声も出なくなる)。
                        #   独立コネクションでのraw SQL INSERTに変更し、
                        #   失敗しても呼び出し元セッションを汚染しないようにする。
                        try:
                            _now = datetime.utcnow()
                            with engine.connect() as _il_conn:
                                with _il_conn.begin():
                                    _il_conn.execute(text(
                                        "INSERT INTO user_interest_logs "
                                        "(user_uuid, category, keyword, mention_count, "
                                        "sentiment, last_mentioned, first_mentioned) "
                                        "VALUES (:u, :c, :k, 1, :s, :t, :t) "
                                        "ON CONFLICT DO NOTHING"
                                    ), {'u': user_uuid, 'c': category, 'k': kw,
                                        's': sentiment, 't': _now})
                        except Exception as _il_err:
                            logger.warning(f"興味ログINSERT失敗 ({category}/{kw}): {_il_err}")
    except Exception as e:
...
# 【世界観・前提条件】
1. **固有名詞の解釈ルール（文脈優先）**:
   - 【人物データ】や【ホロライブ最新ニュース】に登場する名前 → ホロライブ関連として扱う
   - ユーザーが「セカンドライフの〇〇」「SLの〇〇」と明示 → セカンドライフの文脈で解釈する
   - ユーザーが「〇〇について調べて」「〇〇とは」と言った場合で、ホロライブでもSLでもない固有名詞・用語 → **ホロライブ関連と断定せず、一般的な意味で解釈してWeb検索結果を参照する**
   - 文脈が不明な固有名詞は、会話の流れから判断し、断定できない場合はユーザーに確認する
2. **「〇〇について調べて」への対応**:
   - ホロライブ関連 → 【ホロライブ最新ニュース】【人物データ】を参照して回答
   - セカンドライフ関連 → 【セカンドライフ最新情報】を参照して回答
   - それ以外（一般的な用語・技術・ゲーム・社会など） → 【外部検索結果】を参照して回答（内部DBにない情報はWeb検索結果が正）
...
@app.route('/admin/tasks_status_compact', methods=['GET'])
def admin_tasks_status_compact():
    """
    ★ v33.20.4: GitHub Actions のポーリング用の軽量エンドポイント。
    
    /admin/tasks より軽量で、期限切れタスク数と実行中タスクの簡単な情報だけを返す。
    認証は X-Wake-Token ヘッダー必須。
    ...
    レスポンス例:
    {
        ...
        "running_count": 2,
        "total_count": 18,
        "overdue_tasks": ["fetch_news", "update_holomem", "process_streams"],
        "ready_to_sleep": false
    }
    
    "ready_to_sleep" が true なら GitHub Actions のジョブを終了してOK。
    false なら継続してポーリングする。
    """
    if not check_wake_auth():
        return create_json_response({'error': 'Unauthorized'}, 401)
    ...
    try:
        for task_name, config in TASK_SCHEDULE.items():
            try:
                # 時間帯外のタスクは「期限切れ」と見なさない
                allowed, _ = is_allowed_time_for_task(task_name)
                if not allowed:
                    continue
                ...
                logger.debug(f"tasks_status_compact エラー ({task_name}): {e}")
    except Exception as e:
        logger.error(f"tasks_status_compact 全体エラー: {e}")
    
    # 全タスクの期限切れがゼロなら「スリープOK」
    ready = len(overdue) == 0
    
    return create_json_response({
        'overdue_count': len(overdue),
        'total_count': len(TASK_SCHEDULE),
        'overdue_tasks': overdue,
        'ready_to_sleep': ready,
        'idle': conversation_activity.is_idle(),
        'server_time_utc': datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S'),
    })
...
def chat_lsl():
    try:
        ...
        # ★ スリープ明け1回目のシーケンスズレを防ぐため毎回修正（quiet=Trueで無音実行）
        try:
            fix_postgres_sequences(quiet=True)
        except Exception:
            pass
...
def fix_postgres_sequences(quiet: bool = False):
    ...
    v33.20.4: アドバイザリーロックで並列INSERTとの競合を解消
    - PostgreSQL の pg_advisory_lock を使い、シーケンス修正中は他のINSERTを待たせる
    - これがないと、修正直後に他スレッドがINSERT → 再びズレる、という競合が発生
    - lockキー 9876543210 はもちこAI専用の固定値（他システムと衝突しない値）
    """
    if 'sqlite' in str(DATABASE_URL):
        return

    if not quiet:
        logger.info("🔧 DBの連番ズレを修正中（アドバイザリーロック取得）...")
    ...
    LOCK_KEY = 9876543210  # もちこAI 専用のadvisory lockキー
    fixed = 0
    failed = 0
    
    # アドバイザリーロック取得（既に他のスレッドが取っていればここで待つ）
    # タイムアウト無し版を使う - シーケンス修正は数秒で終わるので長くは待たない
    lock_conn = None
    try:
        lock_conn = engine.connect()
        # トランザクション外で取るセッションロック（接続が閉じるまで保持）
        lock_conn.execute(text(f"SELECT pg_advisory_lock({LOCK_KEY})"))
        if not quiet:
            logger.info(f"  🔒 アドバイザリーロック取得: key={LOCK_KEY}")
        ...
        for table in tables:
            try:
                ...
                with engine.connect() as conn:
                    with conn.begin():
                        # ★ v33.21: setval は strict関数のため、シーケンスが存在せず
                        #   pg_get_serial_sequence() が NULL を返すと、setval(NULL,...) は
                        #   例外を出さず黙って NULL を返す = 「✅修正完了」と誤報告していた。
                        #   先にシーケンスの存在を確認し、無ければ明示的に失敗させる。
                        seq_name = conn.execute(text(
                            f"SELECT pg_get_serial_sequence('public.{table}', 'id')"
                        )).scalar()
                        if not seq_name:
                            raise RuntimeError("シーケンス未定義 (pg_get_serial_sequence=NULL)")
                        # MAX(id) を bigint にキャスト（id列が整数型でない場合の
                        # 「function setval(text, double precision, boolean) does not exist」対策）
                        conn.execute(text(
                            f"SELECT setval('{seq_name}', "
                            f"COALESCE((SELECT MAX(id) FROM {table}), 1)::bigint, true)"
                        ))
                        fixed += 1
                        if not quiet:
                            logger.info(f"  ✅ {table}: シーケンス修正完了 (seq={seq_name})")
            except Exception as e:
                failed += 1
                if not quiet:
                    logger.warning(f"  ⚠️ {table}スキップ: {str(e)[:100]}")
    finally:
        # アドバイザリーロック解放
        if lock_conn is not None:
            try:
                lock_conn.execute(text(f"SELECT pg_advisory_unlock({LOCK_KEY})"))
                if not quiet:
                    logger.info(f"  🔓 アドバイザリーロック解放: key={LOCK_KEY}")
            except Exception:
                pass
            try:
                lock_conn.close()
...
def repair_missing_id_sequences():
    """
    ★ v33.22: id列の自動採番機構が失われたテーブルを修復する。

    【原因】
    Excelバックアップ (backup_YYYYMMDD.xlsx) から pandas.to_sql 等で
    テーブルを復元した際、pandas は
      - int64 → BIGINT にマッピングし
      - シーケンス / DEFAULT / IDENTITY / 主キーを一切作らない
    ため、id列が「採番できないただのbigint」になっていた。
    その結果 INSERT時に id=NULL となり、SQLAlchemyが
    "Instance <X> has a NULL identity key" (FlushError) を投げ、
    セッションが汚染されて以降の全クエリが巻き添えで落ちていた。

    【修復内容】(対象テーブルごとに独立トランザクション)
      1. id列が整数型でなければ bigint にキャスト
      2. id が NULL の行に採番し直す
      3. id を NOT NULL 化
      4. GENERATED BY DEFAULT AS IDENTITY を付与 (= 自動採番の復活)
      5. シーケンスを MAX(id)+1 から開始するよう調整

    冪等: 既に identity/default/sequence があるテーブルは対象外なので
    何度実行しても安全。
    """
    if 'sqlite' in str(DATABASE_URL):
        return
    ...
    try:
        with engine.connect() as conn:
            # ★ v33.23: pg_get_serial_sequence() を WHERE句に置いてはいけない。
            #   PostgreSQLのプランナはWHERE条件を任意の順序で評価してよいため、
            #   c.table_schema='public' より先に関数が評価され、
            #   information_schema.columns に含まれる pg_catalog のテーブル
            #   (pg_statistic 等) に対して実行されて
            #   「column "id" of relation "pg_statistic" does not exist」で落ちる。
            #   → 実績のある diagnose_id_column_defaults() と同じく SELECT句に置き、
            #     絞り込みは Python 側で行う。
            rows = conn.execute(text(
                ...
                "       COALESCE(pg_get_serial_sequence(c.table_name, 'id'), '') "
        ...
        targets = [
            (tbl, dtype)
            for tbl, dtype, is_ident, coldef, seq in rows
            if is_ident != 'YES' and not coldef.startswith('nextval') and not seq
        ]
    except Exception as e:
        logger.error(f"❌ id列修復: 対象テーブルの特定に失敗: {e}")
        return

    if not targets:
        logger.info("✅ id列修復: 対象なし (全テーブル採番正常)")
        return

    logger.warning(f"🔧 id列修復開始: {len(targets)}件 {[t[0] for t in targets]}")
    ok = 0
    ng = 0
    for tbl, dtype in targets:
        # --- 段階1: 型の矯正 + NULL採番 + NOT NULL 化 (独立トランザクション) ---
        try:
            with engine.connect() as conn:
                with conn.begin():
                    # 整数型でなければ bigint へ (hololive_news の double precision 対策)
                    if dtype not in ('integer', 'bigint', 'smallint'):
                        conn.execute(text(
                            f"ALTER TABLE {tbl} ALTER COLUMN id TYPE bigint USING id::bigint"
                        ))
                        logger.info(f"  ・{tbl}: id型を {dtype} → bigint に変換")

                    # id が NULL の行に連番を振る (ctid で行を一意特定)
                    null_cnt = conn.execute(text(
                        f"SELECT COUNT(*) FROM {tbl} WHERE id IS NULL"
                    )).scalar()
                    if null_cnt:
                        conn.execute(text(
                            f"UPDATE {tbl} AS tgt SET id = s.newid FROM ("
                            f"  SELECT ctid AS cid, "
                            f"         (SELECT COALESCE(MAX(id), 0) FROM {tbl}) "
                            f"         + row_number() OVER () AS newid "
                            f"  FROM {tbl} WHERE id IS NULL"
                            f") AS s WHERE tgt.ctid = s.cid"
                        ))
                        logger.info(f"  ・{tbl}: id=NULL の {null_cnt}行を採番")

                    conn.execute(text(f"ALTER TABLE {tbl} ALTER COLUMN id SET NOT NULL"))
        except Exception as e:
            ng += 1
            logger.error(f"  ❌ {tbl}: 段階1(型/NOT NULL)で失敗: {e}")
            continue

        # 次に払い出すべき id
        try:
            with engine.connect() as conn:
                nextid = int(conn.execute(text(
                    f"SELECT COALESCE(MAX(id), 0) + 1 FROM {tbl}"
                )).scalar())
        except Exception as e:
            ng += 1
            logger.error(f"  ❌ {tbl}: MAX(id)取得で失敗: {e}")
            continue

        # --- 段階2: 方式A = IDENTITY (独立トランザクション) ---
        method = None
        err_identity = None
        try:
            with engine.connect() as conn:
                with conn.begin():
                    conn.execute(text(
                        ...
                        f"ADD GENERATED BY DEFAULT AS IDENTITY (START WITH {nextid})"
                    ))
            method = 'IDENTITY'
        except Exception as e:
            err_identity = e

        # --- 段階3: 方式B = 従来のシーケンス方式 (Aが失敗した場合のみ) ---
        if method is None:
            seq = f"{tbl}_id_seq"
            try:
                with engine.connect() as conn:
                    with conn.begin():
                        conn.execute(text(
                            f'CREATE SEQUENCE IF NOT EXISTS "{seq}" OWNED BY {tbl}.id'
                        ))
                        conn.execute(text(
                            f"SELECT setval('{seq}', {nextid}, false)"
                        ))
                        conn.execute(text(
                            f"ALTER TABLE {tbl} ALTER COLUMN id "
                ...
                ng += 1
                logger.error(f"  ❌ {tbl}: 方式A(IDENTITY)失敗: {err_identity}")
                logger.error(f"  ❌ {tbl}: 方式B(SEQUENCE)も失敗: {e}")
                continue

        ok += 1
        logger.info(f"  ✅ {tbl}: 自動採番を復旧 ({method}, 次のid={nextid})")

    logger.warning(f"🔧 id列修復完了: {ok}件成功 / {ng}件失敗")

def reconcile_column_types():
    """
    ★ v33.22: モデル定義と実際の列型を突き合わせ、pandas復元で壊れた列型を矯正する。

    【原因】
    pandas.to_sql は Excel の中身から型を推論するため、
    「全行が空だった列」は float64 と推論され DOUBLE PRECISION で作られてしまう。
    例: background_tasks.result (本来Text) が double precision になり、
        日本語の応答を保存しようとして
        "invalid input syntax for type double precision" で落ちていた。

    【安全性】
    これらの列が float 化したのは「中身が空だったから」なので、
    データが1件でも入っている列は対象外とする(誤変換防止)。
    → 失われるデータは無い。
    """
    if 'sqlite' in str(DATABASE_URL):
        return
    ...
    try:
        with engine.connect() as conn:
            actual = {}
            for tname, cname, dtype in conn.execute(text(
                ...
                actual[(tname, cname)] = dtype
    except Exception as e:
        logger.error(f"❌ 列型突合: 実際の型の取得に失敗: {e}")
        return

    # モデル定義(Base.metadata)を正として、型が食い違う列を洗い出す
    mismatches = []
    for table in Base.metadata.sorted_tables:
        for col in table.columns:
            act = actual.get((table.name, col.name))
            if act is None:
                continue
            try:
                expected_sql = col.type.compile(engine.dialect)
            except Exception:
                continue
            # pandas が壊すのは「本来 float でない列が double precision になる」パターンのみ
            if act == 'double precision' and 'DOUBLE' not in expected_sql.upper() \
                    and 'FLOAT' not in expected_sql.upper() and 'REAL' not in expected_sql.upper():
                mismatches.append((table.name, col.name, expected_sql))

    if not mismatches:
        logger.info("✅ 列型突合: 型の食い違いなし")
        return

    logger.warning(f"🔧 列型矯正開始: {len(mismatches)}列が double precision 化しています")
    ok = 0
    ng = 0
    skip = 0
    for tbl, col, expected_sql in mismatches:
        try:
            with engine.connect() as conn:
                notnull = conn.execute(text(
                    f'SELECT COUNT(*) FROM {tbl} WHERE "{col}" IS NOT NULL'
                )).scalar()
            if notnull:
                skip += 1
                logger.warning(
                    f"  ⏭️ {tbl}.{col}: データが{notnull}件あるため自動変換しません "
                    f"(手動確認が必要)"
                )
                continue
            with engine.connect() as conn:
                with conn.begin():
                    # 全行NULLなので USING NULL で安全に型だけ差し替える
                    conn.execute(text(
                        f'ALTER TABLE {tbl} ALTER COLUMN "{col}" '
                        f'TYPE {expected_sql} USING NULL::{expected_sql}'
                    ))
            ok += 1
            logger.info(f"  ✅ {tbl}.{col}: double precision → {expected_sql}")
        except Exception as e:
            ng += 1
            logger.error(f"  ❌ {tbl}.{col}: 型矯正に失敗: {e}")

    logger.warning(f"🔧 列型矯正完了: {ok}件成功 / {ng}件失敗 / {skip}件スキップ")

def diagnose_id_column_defaults():
    """
    ★ v33.21: 各テーブルの id 列に SERIAL 由来の DEFAULT nextval(...) が
    設定されているかを診断する。

    DEFAULT が無いテーブルは INSERT時に id を採番できず、SQLAlchemy が
    "Instance <X> has a NULL identity key" (FlushError) を投げる。
    これが起きるとセッションに壊れたオブジェクトが滞留し、以降の全クエリが
    autoflushで同じ例外を投げ続ける（= chat_lsl がフォールバック応答になり
    音声が出なくなる障害の根本原因）。

    fix_postgres_sequences() の setval はシーケンスが存在しないテーブルには
    無力なため、この診断で「シーケンス自体が無い」ケースを切り分ける。
    """
    if 'sqlite' in str(DATABASE_URL):
        return
    try:
        ...
        logger.info("🔎 id列 精密診断 (type / identity / default / sequence)")
        broken = []
        badtype = []
        for tbl, dtype, is_ident, idgen, coldef, seq in rows:
            has_identity = (is_ident == 'YES')
            has_default = coldef.startswith('nextval')
            has_seq = (seq != '-')
            ok = has_identity or has_default or has_seq
            if dtype not in ('integer', 'bigint', 'smallint'):
                badtype.append(f"{tbl}({dtype})")
            if not ok:
                broken.append(tbl)
            mark = '✅' if ok else '🚨'
            logger.info(
                f"  {mark} {tbl}: type={dtype} identity={is_ident}/{idgen} "
                f"default={coldef[:38]} seq={seq}"
            )

        if broken:
            logger.error(f"🚨 採番不能(identity無し/default無し/sequence無し) {len(broken)}件: {broken}")
            logger.error("   → INSERT時に id=NULL となり NULL identity key の直接原因です")
        else:
            logger.info(f"✅ id列採番: {len(rows)}テーブル全て正常(identity または nextval あり)")

        if badtype:
            logger.error(f"🚨 id列が整数型でないテーブル: {badtype}")
    except Exception as e:
        logger.warning(f"⚠️ id列精密診断スキップ: {e}")
