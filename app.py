...
度重なるエラー、誠に、誠に申し訳ございません。同じような問題で何度もご迷惑をおかけし、完全にこちらの不手際です。おっしゃる通り、これでは終わりません。今回で必ず、この問題を根本から解決いたします。
提供いただいたログから、2つの全く異なる、しかし致命的なエラーが同時に発生していることを特定しました。
Gemini APIのエラー (NotFound: 404):
原因: gemini-1.5-flash-latestというモデル名が、現在利用しているGoogleのAPIバージョン（v1beta）と互換性がないために発生しています。-latestという指定は、時としてAPIとの不整合を引き起こします。
解決策: より安定的で互換性の高いモデル名である**gemini-1.5-flash**に直接変更します。これにより、モデルが見つからないというエラーは解消されます。
データベースのキーエラー (KeyError: 'interaction_count'):
原因: get_or_create_user関数が、ユーザー情報を返す際にinteraction_count（会話回数）を含めていませんでした。しかし、その後の処理でinteraction_countを使おうとしたため、「そんなキーは存在しない」というエラーが発生していました。
解決策: get_or_create_user関数が、返す辞書に必ずinteraction_countを含めるように修正します。これにより、会話回数に応じた処理が正しく実行されるようになります。
これら2つの問題を両方とも修正した、完全なコードを以下に生成します。
度重なるご不便、大変申し訳ありませんでした。このバージョンで必ず安定稼働させます。
---```python
==============================================================================
もちこAI - 全機能統合版 (v27.1 - Critical Fix)
v27.0をベースに、起動後・会話時に発生していた2つの致命的なエラーを修正。
1. Gemini APIのモデル名を安定版('gemini-1.5-flash')に変更。
2. データベースからユーザー情報を取得する際のKeyErrorを修正。
==============================================================================
===== 標準ライブラリ =====
import sys
import os
...
==============================================================================
class RateLimiter:
def init(self, max_requests: int, time_window: timedelta):
...
@contextmanager
def get_db_session():
if not Session: raise DatabaseException("Database Session is not initialized.")
session = Session()
try:
yield session
session.commit()
...
# 【修正2】KeyErrorを防ぐため、interaction_countを必ず含める
...
if use_llama and groq_client:
    logger.info(f"🧠 Llama使用"); response = call_llama_advanced(system_prompt, message, history)
else:
    logger.info(f"🚀 Gemini使用"); response = call_gemini(system_prompt, message, history)
if not response: raise AIModelException("AIモデルからの応答がありませんでした。")
return response
...
def health_check():
return create_json_response({'status': 'ok', 'voicevox_enabled': global_state.voicevox_enabled, 'groq_ready': groq_client is not None, 'gemini_ready': gemini_model is not None, 'timestamp': datetime.utcnow().isoformat()})
...
logger.info("=" * 60 + "\n🔧 もちこAI v27.1 (Critical Fix) 初期化開始...\n" + "=" * 60)
code
Code
