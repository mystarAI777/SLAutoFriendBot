...
import osimport requestsimport loggingimport sysimport timeimport threadingimport jsonimport refrom datetime import datetimefrom flask import Flask, request, jsonify, send_from_directoryfrom flask_cors import CORSfrom sqlalchemy import create_engine, Column, String, DateTime, Integerfrom sqlalchemy.orm import declarative_base, sessionmakerfrom urllib.parse import quote_plus, urljoinfrom bs4 import BeautifulSoup
ログ設定を詳細に
...
def get_secret(name):    secret_file = f'/etc/secrets/{name}'    try:        with open(secret_file, 'r') as f:            value = f.read().strip()            logger.info(f"Secret FileからGOT {name[:4]}***を読み込みました。")            return value    except FileNotFoundError:        logger.warning(f"Secret File '{secret_file}' が見つかりません。環境変数を試します。")        return os.environ.get(name)    except Exception as e:        logger.error(f"{name} の読み込み中にエラー: {e}")        return None
...
class UserMemory(Base):    tablename = 'user_memories'    id = Column(Integer, primary_key=True)    user_uuid = Column(String(255), unique=True, nullable=False)    user_name = Column(String(255), nullable=False)    interaction_count = Column(Integer, default=0)
Base.metadata.create_all(engine)Session = sessionmaker(bind=engine)
★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★
★ 強化されたWeb検索機能 - Yahoo!ニュース & Wikipedia対応 ★
★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★
def clean_text(text):    """HTMLタグを除去し、テキストをクリーンアップ"""    if not text:        return ""    # HTMLタグを除去    text = re.sub(r'<[^>]+>', '', text)    # 複数の空白を1つに    text = re.sub(r'\s+', ' ', text)    # 改行を空白に    text = text.replace('\n', ' ').replace('\r', ' ')    return text.strip()
def search_yahoo_news(query):    """Yahoo!ニュースから最新情報を検索"""    try:        search_url = f"https://news.yahoo.co.jp/search?p={quote_plus(query)}&ei=UTF-8"        headers = {            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'        }
...
def search_wikipedia(query):    """Wikipedia日本語版から情報を検索"""    try:        # Wikipedia API を使用        api_url = "https://ja.wikipedia.org/api/rest_v1/page/summary/" + quote_plus(query)        headers = {            'User-Agent': 'Mozilla/5.0 (compatible; AI-Assistant/1.0)'        }
...
def search_google_basic(query):    """基本的なGoogle検索（スクレイピング）"""    try:        search_url = f"https://www.google.com/search?q={quote_plus(query)}&hl=ja&lr=lang_ja"        headers = {            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'        }
    response = requests.get(search_url, headers=headers, timeout=8)
    response.raise_for_status()
...
def enhanced_web_search(query):    """強化されたWeb検索 - 複数ソースから情報を収集"""    logger.info(f"🔍 強化Web検索開始: '{query}'")
results = []
...
def should_search(message: str) -> bool:    """メッセージがWeb検索を必要とするかを判定する（改良版）"""    # より具体的な検索キーワードパターン    search_patterns = [        # 疑問詞        r'(?:誰|何|どこ|いつ|どう|なぜ|どの).{0,10}(?:ですか|だっけ|？|?)',        # 説明要求        r'(?:とは|って何|について|教えて|知りたい)',        # 最新情報        r'(?:最新|今日|昨日|最近|ニュース|現在)',        # 具体的な質問        r'(?:どうなった|結果|状況|株価|天気|為替)',    ]
# パターンマッチング
...
class UserDataContainer:    def init(self, user_uuid, user_name, interaction_count):        self.user_uuid = user_uuid        self.user_name = user_name        self.interaction_count = interaction_count
def get_or_create_user(user_uuid, user_name):    session = Session()    try:        user_memory = session.query(UserMemory).filter_by(user_uuid=user_uuid).first()        if user_memory:            user_memory.interaction_count += 1        else:            user_memory = UserMemory(user_uuid=user_uuid, user_name=user_name, interaction_count=1)            session.add(user_memory)        session.commit()        return UserDataContainer(user_memory.user_uuid, user_memory.user_name, user_memory.interaction_count)    finally:        session.close()
def generate_ai_response(user_data, message):    """【強化版】Web検索結果をプロンプトに含めてAI応答を生成"""    if not groq_client:        return "あてぃし、今ちょっと調子悪いかも...またあとで話そ！"
search_info = ""
...
検索結果を無視せず、必ず活用してください"""
  try:  logger.info(f"🤖 Groqに応答生成をリクエストします。")  completion = groq_client.chat.completions.create(      messages=[          {"role": "system", "content": system_prompt},          {"role": "user", "content": message or "ねぇねぇ、元気？"}      ],      model="llama3-8b-8192",      temperature=0.7,      max_tokens=120,  )  response = completion.choices[0].message.content.strip()  # 検索情報が活用されているかチェック  if search_info and search_info not in ["なし", ""] and len(search_info) > 50:      logger.info(f"✅ 検索情報を含む応答を生成しました")  return response  except Exception as e:  logger.error(f"AI応答生成エラー: {e}")  return "あてぃし、ちょっと調子悪いかも...またあとで話そ！"
...
voice_files = {}voice_files_lock = threading.Lock()
def store_voice_file(filename, voice_data):    try:        os.makedirs(VOICE_DIR, exist_ok=True)        filepath = os.path.join(VOICE_DIR, filename)
    # ファイルを先にディスクに保存
...
def background_voice_generation(text, filename, speaker_id=3):    logger.info(f"🎤 バックグラウンド音声生成開始: {filename}")
# 生成中ステータスを先に登録
with voice_files_lock:
    voice_files[filename] = {
        'data': None, 
...
@app.route('/voice/')def serve_voice(filename):    try:        logger.info(f"🎵 音声ファイル要求: {filename}")
    # メモリキャッシュをまずチェック
    with voice_files_lock:
        if filename in voice_files:
