"""
==============================================================================
もちこAI - 全機能統合版 (v28.0 - Complete Edition)
v27.1をベースに、以下を完全実装:
1. コードの省略を完全に解消
2. バックグラウンド会話継続機能（検索中も会話可能）
3. アニメ専門検索の強化（専門サイト失敗時にWEB検索フォールバック）
==============================================================================
"""
...
SPECIALIZED_SITES = {
    'Blender': {
        ...
        'keywords': ['Blender', 'ブレンダー', 'blener']
    },
    'CGニュース': {
        'base_url': 'https://modelinghappy.com/',
        'keywords': ['CGニュース', '3DCG', 'CG業界']
    },
    '脳科学・心理学': {
        'base_url': 'https://nazology.kusuguru.co.jp/',
        'keywords': ['脳科学', '心理学', '脳', '認知科学']
    },
    'セカンドライフ': {
...
HOLOMEM_KEYWORDS = [
    ...
    '宝鐘マリン', '天音かなた', '角巻わため', '常闇トワ', '姫森ルーナ', '雪花ラミィ', '桃鈴ねね',
    '獅白ぼたん', '尾丸ポルカ', 'ラプラス・ダークネス', '鷹嶺ルイ', '博衣こより', '沙花叉クロヱ',
    '風真いろは', '森カリオペ', '小鳥遊キアラ', '一伊那尓栖', 'がうる・ぐら', 'ワトソン・アメリア',
    'IRyS', 'セレス・ファウナ', 'オーロ・クロニー', '七詩ムメイ', 'ハコス・ベールズ', 'シオリ・ノヴェラ',
    '古石ビジュー', 'ネリッサ・レイヴンクロフト', 'フワワ・アビスガード', 'モココ・アビスガード',
    'アユンダ・リス', 'ムーナ・ホシノヴァ', 'アイラニ・イオフィフティーン', 'クレイジー・オリー',
    'アーニャ・メルフィッサ', 'パヴォリア・レイネ', '火威青', '音乃瀬奏', '一条莉々華',
    '儒烏風亭らでん', '轟はじめ', 'ホロライブ', 'ホロメン', 'hololive', 'YAGOO',
    '桐生ココ', '潤羽るしあ', '魔乃アロエ', '九十九佐命'
]

ANIME_KEYWORDS = [
    'アニメ', 'anime', 'ANIME', 'ｱﾆﾒ', 'アニメーション', '作画', '声優',
    'OP', 'ED', '劇場版', '映画', '原作', '漫画', 'ラノベ'
]
...
def is_weather_request(message):
    weather_keywords = ['今日の天気', '明日の天気', '天気予報']
...
def is_explicit_search_request(message):
    search_keywords = ['調べて', '検索して', '探して', 'とは', 'って何', 'について', '教えて']
...
def is_short_response(message):
    ...
    return len(normalized_message) <= 5 or normalized_message in short_responses
...
def call_gemini(system_prompt, message, history):
    ...
    try:
        ...
        for h in history:
            role_name = 'ユーザー' if h['role'] == 'user' else 'もちこ'
...
def analyze_user_psychology(user_uuid):
    ...
    with get_db_session() as session:
        try:
            ...
            analysis_prompt = f"""以下のユーザーの発言履歴を分析し、ビッグファイブ理論に基づいた性格特性を0〜100の数値で評価してください。
また、興味、会話スタイル、感情の傾向を分析し、総合的なサマリーを生成してください。
...
{{"openness":50,"conscientiousness":50,"extraversion":50,"agreeableness":50,"neuroticism":50,"interests":[],"favorite_topics":[],"conversation_style":"","emotional_tendency":"","analysis_summary":"","analysis_confidence":75}}"""
            ...
            response_text = call_llama_advanced(
                "あなたは優秀な心理学者です。",
                analysis_prompt,
                [],
                max_tokens=1024
...
def get_psychology_insight(session, user_uuid):
    ...
    insights = []
    if psych.extraversion > 70:
        insights.append("社交的な")
    if psych.openness > 70:
        insights.append("好奇心旺盛な")
    if psych.conversation_style:
        insights.append(f"{psych.conversation_style}スタイルの")
    
    try:
        favorite_topics = json.loads(psych.favorite_topics) if psych.favorite_topics else []
        if favorite_topics:
            topics_str = '、'.join(favorite_topics[:2])
            insights.append(f"{topics_str}が好きな")
...
def get_weather_forecast(location):
    ...
    try:
        ...
        target_area = data.get('targetArea', location)
        text = clean_text(data.get('text', ''))
        
        return f"今の{target_area}の天気はね、「{text}」って感じだよ！"
    except Exception as e:
        ...
        try:
            ...
            if wiki:
                ...
                if db_field and hasattr(wiki, db_field):
                    ...
                    result = f"ごめん、「{correction_data['field']}」っていう項目は修正できないみたい…"
            else:
                result = f"ごめん、「{correction_data['member_name']}」がデータベースに見つからなかった…"
        ...
        with get_db_session() as session:
            for status, section in member_sections.items():
                ...
                for element in section.find_all(['h3', 'a']):
                    if element.name == 'h3':
                    ...
                    elif element.name == 'a' and 'title' in element.attrs:
                        if element.find_parent('h3'):
                            continue
                        ...
                        existing_member = session.query(HolomemWiki).filter_by(
                            member_name=member_name
                        ).first()
...
def scrape_major_search_engines(query, num_results=3, site_filter=None):
    ...
    engines = [
        ...
        {
            'name': 'Yahoo',
            'url': f"https://search.yahoo.co.jp/search?p={quote_plus(search_query)}",
            'selector': 'div.sw-CardBase',
            'title_sel': 'h3.sw-Card__title',
            'snippet_sel': 'div.sw-Card__summary'
        },
        {
            'name': 'DuckDuckGo',
    ...
    with get_db_session() as session:
        try:
            ...
            if search_type == 'hololive_search':
                results = scrape_major_search_engines(
                    query, 5, site_filter="seesaawiki.jp/hololivetv/"
                )
                if not results:
                ...
                results = scrape_major_search_engines(
                    query, 3, site_filter=site_url_domain
                )
                
                if not results:
                    logger.info(f"専門サイト検索失敗。一般WEB検索にフォールバック...")
                    results = scrape_major_search_engines(query, 5)
                ...
                if anime_site:
                    ...
                    results = scrape_major_search_engines(
                        query, 3, site_filter=site_domain
                    )
                
                if not results:
                    logger.info(f"アニメ専門サイト検索失敗。一般WEB検索にフォールバック...")
                    results = scrape_major_search_engines(query, 5)
            ...
            logger.error(f"❌ バックグラウンド検索タスクエラー: {e}", exc_info=True)
...
def generate_ai_response(user_data, message, history, reference_info="", is_detailed=False, is_task_report=False):
    use_llama = (
        is_detailed or
        is_task_report or
        len(reference_info) > 100 or
        any(kw in message for kw in ['分析', '詳しく', '説明して', 'なぜ'])
    )
...
- もし情報が見つからなくても、「わかりません」で終わらせず、新しい話題を提案して会話を続けて！"""
    ...
    if use_llama and groq_client:
        logger.info(f"🧠 Llama使用")
        response = call_llama_advanced(system_prompt, message, history)
    else:
        logger.info(f"🚀 Gemini使用")
...
def health_check():
    return create_json_response({
        'status': 'ok',
        'voicevox_enabled': global_state.voicevox_enabled,
    ...
    try:
        ...
        with get_db_session() as session:
            ...
            if correction:
                ...
                ai_text = f"まじ！？「{correction['member_name']}」ちゃんの情報、教えてくれてありがと！裏で直しとくね！"
            ...
            if not ai_text:
                ...
                if member_name:
                    ...
                    if info:
                        ...
                        ai_text = generate_ai_response_safe(
                            user_data,
                            f"{member_name}について教えて！",
                            history,
                            reference_info=reference,
                            is_detailed=True
                        )
                    else:
                        ai_text = f"{member_name}ちゃん？ごめん、あてぃしのデータにないみたい…新しい子かな？"
            ...
            if not ai_text and not is_short_response(message):
                if is_hololive_request(message) and is_explicit_search_request(message):
                    ...
                    ai_text = f"ホロライブのことだね！Wikiとかで詳しく探してくるから、ちょっと待ってて！他に何か話したいことある？"
                    is_task_started = True
                    ...
                    session.add(task)
                    background_executor.submit(background_deep_search, task_id, query_data)
                    ai_text = f"アニメの話だね！まじ好き！詳しく調べてくるから待っててね！てか、最近何か面白いの見た？"
                    is_task_started = True
                
                else:
                    specialized_topic = detect_specialized_topic(message)
                    if specialized_topic:
                        site_info = SPECIALIZED_SITES[specialized_topic]
                        task_id = f"search_{user_uuid}_{int(time.time())}"
                        query_data = {
                            'query': message,
                            'user_data': user_data,
                        ...
                        session.add(task)
                        background_executor.submit(background_deep_search, task_id, query_data)
                        ai_text = f"{specialized_topic}の話？まじ！？ちょっと詳しく調べてくるから待ってて～！その間に他の話もしよ！"
                        is_task_started = True
                        ...
                        session.add(task)
                        background_executor.submit(background_deep_search, task_id, query_data)
                        ai_text = "オッケー！その話、ちょっとググってくるから待ってて！てか、それについて何が知りたいの？"
                        is_task_started = True
            
            if not ai_text:
                ref_info = ""
                news = (
                    session.query(HolomemNews).order_by(HololiveNews.created_at.desc())
                    .limit(3)
                    .all()
                )
                if is_hololive_request(message) and news:
                    ref_info = "最近のホロライブニュース:\n" + "\n".join([
        ...
        logger.error(f"❌ Chatエンドポイントで予期せぬエラー: {e}", exc_info=True)
        return Response(
            "ごめん、システムエラーが起きちゃった…|",
            mimetype='text/plain; charset=utf-8',
            status=500
...
def generate_voice_file(text, user_uuid):
    ...
    try:
        query_res = requests.post(
            f"{global_state.active_voicevox_url}/audio_query",
        ...
        synth_res = requests.post(
            f"{global_state.active_voicevox_url}/synthesis",
    ...
    try:
        ...
        for filepath in glob.glob(os.path.join(VOICE_DIR, '*.wav')):
            if os.path.getmtime(filepath) < cutoff_time:
                os.remove(filepath)
                logger.info(f"古い音声ファイルを削除: {os.path.basename(filepath)}")
    except Exception as e:
...
def initialize_app():
    ...
    logger.info("=" * 60)
    logger.info("🔧 もちこAI v28.0 (Complete Edition) 初期化開始...")
    logger.info("=" * 60)
    ...
    if GEMINI_API_KEY:
        ...
        logger.info("✅ Gemini API初期化完了 (model: gemini-1.5-flash)")
    else:
