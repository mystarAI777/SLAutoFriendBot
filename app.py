...
def diagnose_id_column_defaults():
    ...
    try:
        ...
        broken = [r[0] for r in rows if not r[1]]
        if broken:
            logger.error(f"🚨 id列にDEFAULT(nextval)が無いテーブル {len(broken)}件: {broken}")
            logger.error("   → これらは INSERT時に id=NULL となり NULL identity key の原因になります")
            logger.error("   → 対処: ALTER TABLE <table> ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY;")
        else:
            logger.info(f"✅ id列DEFAULT診断: {len(rows)}テーブル全て正常")
    except Exception as e:
        logger.warning(f"⚠️ id列DEFAULT診断スキップ: {e}")
